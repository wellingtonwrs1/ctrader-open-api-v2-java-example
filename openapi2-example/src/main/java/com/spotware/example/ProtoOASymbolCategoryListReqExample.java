...
public class ProtoOASymbolCategoryListReqExample {
	...
	private static void sendProtoOAGetAccountListByAccessTokenReq(NettyClient nettyClient) throws InterruptedException {
		try {
			...
			ProtoOASymbolCategoryListReq protoOASymbolCategoryListReq = ProtoOASymbolCategoryListReq
					.newBuilder()
					.setCtidTraderAccountId(config.getCtid())
					.build();

			ProtoMessageReceiver receiver = nettyClient.writeAndFlush(protoOASymbolCategoryListReq);

			MessageLite messageLite = receiver.waitSingleResult();

			if (messageLite instanceof ProtoOASymbolCategoryListRes) {
				ProtoOASymbolCategoryListRes response = (ProtoOASymbolCategoryListRes) messageLite;
